...
class BloodStockPage:
    ...
    blood_bank_status = (
        By.XPATH,
        "//h3[contains(text(),'Blood Bank Status')]"
    )
    ...
    def blood_group_option(blood_group):
        return (
            By.XPATH,
            f"//li[normalize-space()='{blood_group}']"
        )
    ...
    def dynamic_option(option):
        return (
            By.XPATH,
            f"//li[contains(@class,'select2-results__option') and contains(normalize-space(),'{option}')]"
        )
    ...
    def added_bag_number(bag):
        return (
            By.XPATH,
            f"(//table//td[normalize-space()='{bag}'])[1]"
        )
    issue_date = (
        By.XPATH,
        "//label[contains(text(),'Issue Date')]"
    )
