...
class SearchPatientPage:
    ...
    search_box        = (By.XPATH, "//input[@type='search' or contains(@placeholder,'Search') or contains(@placeholder,'search')]")
    @staticmethod
    def search_result_name(name):
        return (By.XPATH,
                f"//table//tbody//tr[not(contains(@style,'display: none'))]//td[contains(normalize-space(.), '{name}')]")

    no_data_message   = (By.XPATH, "//td[normalize-space()='No data available in table']")
